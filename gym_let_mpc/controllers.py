...
def initialize_mpc(model, config, tvp_fun=None, suppress_IPOPT_output=True, linear_solver="MA27"):
    ...
    mpc = mpc_set_objective(mpc, config["objective"], reference=reference, parameters=config["model"].get("parameters", None))
...
def mpc_set_objective(mpc, cost_parameters, reference=None, parameters=None):
    ...
    mpc.set_objective(mterm=costs["mterm"], lterm=costs["lterm"])
...
class ETMPC:
    def __init__(self, mpc_config, lqr_config, mpc_model=None, viewer=None):
        ...
        self.mpc = initialize_mpc(mpc_model, mpc_config, tvp_fun=tvp_fun)
        ...
        self._mpc_state_preds = None
        self._mpc_action_sequence = None
        self.current_input = {u_name: None for u_name in self.input_names}
        self.current_reference = {r_name: mpc_config["reference"][r_name]["value"] for r_name in self.reference_names}
        self.viewer = viewer
        self.history = None
    ...
    def reset(self, state=None, reference=None, constraint=None, tvp=None):
        ...
        self.current_input = {u_name: None for u_name in self.input_names}
        self.history = {"epsilons": [], "state_preds": [], "u_mpc": [], "u_lqr": [], "u_cfs": [], "mpc_compute": [],
                        "inputs": [self.current_input], "references": [self.current_reference],
    ...
    def get_action(self, state, compute_mpc_solution=False, tvp_values=None):  # TODO: This is maybe bad as the name doesnt suggest that it changes the object but it acts closely related to a step function
        ...
        if compute_mpc_solution:
            ...
            self._mpc_state_preds = mpc_get_solution(self.mpc, states="all")
            ...
            epsilon_dict = self._get_state_dict(np.full_like(self._mpc_state_preds[:, 0, :], np.nan))
            ...
            self.history["state_preds"].append(np.full_like(self._mpc_state_preds[:, 0, :], np.nan))
            self.history["u_mpc"].append(mpc_optimal_action)
            self.history["u_lqr"].append(np.full_like(mpc_optimal_action, np.nan))
        else:
            mpc_state_pred = self._mpc_state_preds[:, self.steps_since_mpc_computation + 1]
    ...
    def render(self, show=True):
        ...
        mpc_computation_inds = [t_ind for t_ind, computed in enumerate(self.history["mpc_compute"]) if computed]
        ...
        for pred_i, mpc_computation_t_ind in enumerate(mpc_computation_inds):
            ...
            for line_i, line in enumerate(self.viewer['mpc_graphics'].pred_lines.full):
                ...
                if len(pred_line_data[line_i]["x"]) == 0 or min(line_x) >= max(pred_line_data[line_i]["x"]):
                    ...
                    overlap_point = min(range(len(overlap_data)), key=overlap_data.__getitem__)  # Due to float rounding, the exact number might not be present in the other list
        ...
        if "tvp_lines" in self.viewer:
            ...
            for tvp_name, tvp_lines in self.viewer["tvp_lines"].items():
                ...
                mpc_computation_markers = self.history["mpc_compute"] + [False] * (
                        tvp_data_x.shape[0] - len(self.history["mpc_compute"]))
        ...
        for line_i, (data, line) in enumerate(zip(pred_line_data, self.viewer['mpc_graphics'].pred_lines.full)):
            mpc_computation_markers = self.history["mpc_compute"] + [False] * (
                        len(pred_line_data[line_i]["x"]) - len(self.history["mpc_compute"]))
        ...
        for c_name, c_props in self.constraints.items():
            ...
            self.viewer["constraint_lines"][c_name].set_data(
                [0, len(self.history["mpc_compute"]) * self.mpc.data.meta_data['t_step']],
                [c_val, c_val]
            )
    ...
    def update_reference(self, reference):  # TODO: changing mpc components require setting up new model, thereby throwing away data. Maybe not allow during operation.
        if len(reference) > 0:
            ...
            self.mpc = initialize_mpc(self.mpc.model, self.mpc_config)
        ...
        if len(constraints) > 0:
            ...
            for c_name in constraints:
                ...
                if c_name not in self.constraints:
                    self.constraints[c_name] = {"var_name": c_name_parts[1], "var_type": "_{}".format(c_name_parts[1][0]),
                ...
                for c_i, c in enumerate(self.mpc_config["constraints"]):
                    if c["var_name"] == c_name_parts[1] and c["constraint_type"] == ("upper" if c_name_parts[-1] == "u" else "lower"):
    ...
    def update_model(self, model):
        ...
        self.mpc = initialize_mpc(initialize_mpc_model(self.mpc_config["model"]), self.mpc_config)
